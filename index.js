...
router.post('/posts/new', function(req, res, next) {
    ...
    if("title" in req.body && req.body.title !== null
        ...
        req.pool.getConnection(function(err,connection) {
            ...
            connection.query(query, [req.body.author.u_id,req.body.title,sanitizeHtml(req.body.content)], function(qerr, _rows, fields) {
                if (qerr) {
                    res.sendStatus(500);
                    console.error(qerr);
...
router.post('/comments/new', function(req, res, next) {
  ...
  if("title" in req.body && req.body.title !== null
      ...
      req.pool.getConnection(function(err,connection) {
          ...
          // eslint-disable-next-line max-len
          connection.query(query, [req.body.author.u_id,req.body.title,req.body.content,req.body.post_id], function(qerr, _rows) {
              if (qerr) {
                  res.sendStatus(500);
                  console.error(qerr);
                  connection.release(); // release connection if error
